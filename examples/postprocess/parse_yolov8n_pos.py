...
human_pose_upper_body_adjacent_demo = [
    (5, 7), (7, 9),     # left arm
    (6, 8), (8, 10),    # right arm
    (5, 6), (5, 11), (6, 12),  # shoulders to hips
    (11, 12)            # hip center
]
human_pose_right_lower_body_adjacent_demo = [(12, 14), (14, 16)]  # right leg
human_pose_left_lower_body_adjacent_demo = [(11, 13), (13, 15)]   # left leg
...
def vis(img, boxes, scores, cls_ids, keypoints, class_names=["person"], obejct_detect_conf=0.2, kpt_detect_conf=0.2):
    h, w = img.shape[:2]
    ...
    for i in range(len(boxes)):
        ...
        kps = [KeyPoint(keypoints_raw[j*3], keypoints_raw[j*3+1], keypoints_raw[j*3+2]) for j in range(17)]
        ...
        draw_lines(face_demo)
        draw_lines(human_pose_upper_body_adjacent_demo)
        draw_lines(human_pose_right_lower_body_adjacent_demo)
        draw_lines(human_pose_left_lower_body_adjacent_demo)
        return img


class ParserYolov8nPos:
